...
/**
 * Interface to a discrete domain <code>U</code> (universal set). Domain is
 * built using design pattern Composite. User can ask for cardinality of the
 * domain via {@link #getCardinality()}, number of simple domains this domain is
 * consisted of via {@link #getNumberOfComponents()}, access simple domains by
 * their index {@link #getComponent(int)}. User can also get the index of the
 * element of the domain through {@link #indexOfElement(DomainElement)} and
 * fetch the element at the specified index using {@link #getElement(int)}.
 * 
 * @author freskov
 * @version 1.0
 */
